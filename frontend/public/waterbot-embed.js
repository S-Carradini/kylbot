 ...
 *   { type: 'waterbot:resize', state: 'mascot' | 'nudge' | 'expanded', width?: number, height?: number }
 *     width/height are the widget's own measured content size for the
 *     mascot/nudge states (content-driven, e.g. the "Ask Blue" label).
 *     They're omitted for 'expanded', where the host picks a fixed,
 *     viewport-aware chat-window size instead — see DESKTOP_SIZES below.
...
(function () {
  ...
  var ALLOWED_ORIGIN = (currentScript && currentScript.getAttribute('data-allowed-origin'))  || '*';
  ...
  window.addEventListener('message', function (event) {
    ...
    if (data.type === 'waterbot:resize' && data.state) {
      /* mascot/nudge: trust the widget's own measured size over any guess.
         expanded: the widget doesn't report a size (see protocol note above) —
         always fall through to the host's viewport-aware default for it. */
      var dim = (data.state !== 'expanded' && data.width && data.height)
        ? { w: data.width, h: data.height }
        : getDimensions(data.state);
      iframe.style.width  = dim.w + 'px';
      iframe.style.height = dim.h + 'px';

      /* On mobile, pull the iframe closer to the edge for nudge/expanded states */
      iframe.style.right = (isMobile() && (data.state === 'nudge' || data.state === 'expanded'))
        ? '8px'
        : RIGHT_PX + 'px';
    }
